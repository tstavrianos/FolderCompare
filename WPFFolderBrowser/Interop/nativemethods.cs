...
namespace WPFFolderBrowser.Interop
{
    internal static class NativeMethods
    {
        ...
        internal struct DWM_TIMING_INFO
        {
            ...
            [FieldOffset(20)]
            internal ulong qpcVBlank;          // QPC time at VBlank
            [FieldOffset(28)]
            internal ulong cRefresh;           // DWM refresh counter
            [FieldOffset(36)]
            internal ulong qpcCompose;         // QPC time at a compose time
            [FieldOffset(44)]
            internal ulong cFrame;             // Frame number that was composed at qpcCompose
            [FieldOffset(52)]
            internal ulong cRefreshFrame;      // Refresh count of the frame that was composed at qpcCompose
            [FieldOffset(60)]
            internal ulong cRefreshConfirmed;  // The target refresh count of the last
            // frame confirmed completed by the GPU
            [FieldOffset(68)]
            internal uint cFlipsOutstanding;  // the number of outstanding flips

            //
            // Feedback on previous performance only valid on 2nd and subsequent calls
            //
            [FieldOffset(72)]
            internal ulong cFrameCurrent;      // Last frame displayed
            [FieldOffset(80)]
            internal ulong cFramesAvailable;   // number of frames available but not displayed, used or dropped
            [FieldOffset(88)]
            internal ulong cFrameCleared;      // Source frame number when the following statistics where last cleared
            [FieldOffset(96)]
            internal ulong cFramesReceived;    // number of new frames received
            [FieldOffset(104)]
            internal ulong cFramesDisplayed;   // number of unique frames displayed
            [FieldOffset(112)]
            internal ulong cFramesDropped;     // number of rendered frames that wer  never
